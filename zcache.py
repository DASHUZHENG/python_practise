...
class ZCache():
    ...
    def __getitem__(self, url):
        ...
        if self.category == 1:
            ...
            try:
                da_data = self.db.extract_by_text(
                    table=self.db_setting["table"],
                    att=self.db_setting["att"],
                    key=self.db_setting["key"],
                    value=url)
    ...
    def __setitem__(self, url, html):
        ...
        if self.category == 1:
            ...
            self.db.insert_not_exist(
                table=self.db_setting["table"],
                primekey=self.db_setting["key"],
                subkey=self.db_setting["att"],
                primecontent=url,
                subcontent=1)
    ...
    def refresh(self):
        ...
        if self.category == 1:
            ...
            try:
                self.db.insert_not_exist(
                    table=self.db_log["table"],
                    primekey=self.db_log["key"],
                    subkey=self.db_log["att"],
                    primecontent=self.cache_dir,
                    subcontent=str(
                        int(
                            self.counter)))
    ...
    def _methodset(self):
        ...
        if self.category == 1:
            ...
            try:
                ...
                counter_seed = self.db.extract_by_text(
                    self.db_log["table"],
                    self.db_log["att"],
                    self.db_log["key"],
