...
class zProperty():
    ...
    def __init__(self, name="H2O", temp=298.15, pres=100000, p0=100000, logname="logger", database="zPro.db"):
        ...
        self._tpset = ["SOLIDV", "LIQUIDV", "POV", "HOV",
                       "SOLIDCP", "LIQUIDCP", "GASCP", "VIRIAL"]
        ...
        self._thermoset = ['PHIV', 'PHIL', 'HV', 'HL', 'GV', 'GL', 'SV',
                           'SL', 'VV', 'VL', 'PHIS', 'HS', 'GS', 'SS', 'VS', 'WSL', 'HCSL']
        self._thermosubset = ['PHILPC', 'DHV', 'DHL',
                              'DGV', 'DGL', 'DSV', 'DSL', 'PHISOC']
    ...
    def Basic_property_database(self, database="zPro.db", table="Basic"):
        ...
        for att in self._bpset:
            try:
                ...
                self.logger.info("Function: %s; Name:%s/nError:%s" %
                                 ("Basic_property_database", att, error))
                self.bp[att] = None
        ...
        for att in self._tpset:
            for member in self._tplib[att]:
                try:
                    ...
                    self.logger.info("Function: %s; Name:%s/nError:%s" %
                                     ("Tdep_property_database", att, error))
    ...
    def Tdep_property_set(self, database="zPro.db"):
        ...
        def calcfilter(x): return isinstance(x, float)
        for btt in self.tp:
            try:
                # print self.tp[btt]
                location = "si" + self.tp[btt]

                result2 = db.Locate(database, location,
                                    self.name, "*", "Field1")
...
# T-dep Property
    ...
    def Solid_volume(self, temp, pres, p0=100000):
        equation = zEquation()
        try:
            result = getattr(equation, self.tpdata["SOLIDV"][0])(
                self.tpdata["SOLIDV"], temp, pres, p0)
            result = 1 / result
    ...
    def Liquid_volume(self, temp, pres, p0=100000):
        ...
        equation = zEquation()
        try:
            result = getattr(equation, self.tpdata["LIQUIDV"][0])(
                self.tpdata["LIQUIDV"], temp, pres, p0)
            result = 1 / result
            # DIPPR Returns Density
        except Exception, error:
            print ("KeyWord:%s + %s\nMistake:%s\n" %
    ...
    def Pressure_of_saturation(self, temp, pres, p0=100000):
        ...
        try:
            ...
            result = getattr(equation, self.tpdata["POV"][0])(
                self.tpdata["POV"], temp, pres, p0)
    ...
    def Heat_of_vaporization(self, temp, pres, p0=100000):
        ...
        equation = zEquation()
        try:
            result = getattr(equation, self.tpdata["HOV"][0])(
                self.tpdata["HOV"], temp, pres, p0)
        except Exception, error:
            print ("KeyWord:%s + %s\nMistake:%s\n" %
    ...
    def Solid_cp(self, temp, pres, p0=100000):
        equation = zEquation()
        try:
            result = getattr(equation, self.tpdata["SOLIDCP"][0])(
                self.tpdata["SOLIDCP"], temp, pres, p0)
        except Exception, error:
            print ("KeyWord:%s + %s\nMistake:%s\n" %
    ...
    def Liquid_cp(self, temp, pres, p0=100000):
        equation = zEquation()
        try:
            result = getattr(equation, self.tpdata["LIQUIDCP"][0])(
                self.tpdata["LIQUIDCP"], temp, pres, p0)
        except Exception, error:
            print ("KeyWord:%s + %s\nMistake:%s\n" %
    ...
    def Gas_cp(self, temp, pres, p0=100000):
        equation = zEquation()
        try:
            result = getattr(equation, self.tpdata["GASCP"][0])(
                self.tpdata["GASCP"], temp, pres, p0)
        except Exception, error:
            print ("KeyWord:%s + %s\nMistake:%s\n" %
...
# EOS calculation, thermo property
    ...
    def Phiv(self, temp, pres, p0=100000, route="1"):
        ...
        if route == "1":
            try:
                # function Problem
                def eosphivfunction(x): return (
                    1 / 8.3145 / temp) * (self.Vv(temp, x, p0) / 1000 - 8.3145 * temp / x)
    ...
    def Hv(self, temp, pres, t0=298.15, p0=100000, route="2"):
        ...
        def eoshfunction(x): return self.Gas_cp(x, pres, p0=100000)
    ...
    def Gv(self, temp, pres, t0=298.15, p0=100000, route="2", subroute="2"):
        if route == "1":
            ...
            def cp(x): return self.Gas_cp(x, p0, p0)

            def cpt(y): return self.Gas_cp(y, p0, p0) / y
    ...
    def Gl(self, temp, pres, t0=298.15, p0=100000, route="2", subroute="2"):
        if route == "1":
            ...
            def cpt(y): return self.Gas_cp(x, p0, p0) / x
            deltaH1 = integrate.romberg(cp, t0, temp)
            deltaS1 = integrate.romberg(cpt, t0, temp)
    ...
    def Sv(self, temp, pres, t0=298.15, p0=100000, route="2"):
        ...
        if route == "1":
            ...
            def sfunction(x): return Gas_cp(self, x, pres, p0=100000) / x
            stemp = integrate.romberg(sfunction, t0, temp)
            # spres=8.3145*temp*math.log(pres/p0) this belong to ideal part

            sdeparture = self.Sub_dsv(
                temp, pres, t0=298.15, p0=100000, route="2")
    ...
    def Sl(self, temp, pres, t0=298.15, p0=100000, route="2"):
        try:
            ...
            def eossfunction(x): return self.Gas_cp(x, pres, p0=100000) / x
    ...
    def Vv(self, temp, pres, t0=298.15, p0=100000):
        ...
        try:
            ...
            result = getattr(equation, self.eosset[0])(
                self.eosset, temp, pres, 0, tag="V")
...
# 计算中间属性
    def Sub_philpc(self, temp, p1, p2):
        try:
            def eosphilfunction(x): return self.Liquid_volume(
                temp, x, p0=100000)
    ...
    def Sub_dhv(self, temp, pres, t0=298.15, p0=100000, route="2", step=100):
        ...
        if route == "1":
            ...
            def eosphiv(y): return math.log(self.Phiv(y, pres, p0))
            ...
            for pint in pcomb:
                ...
                def eosdt(x): return self.Vv(x, pint, p0) / 1000
                eosdiff = eosv - temp * \
                    diff.derivative(eosdt, temp, abs(temp - t0) / step)
    ...
    def Sub_dhl(self, temp, pres, t0=298.15, p0=100000, route="1", step=1000):
        ...
        if route == "1":
            try:
                ...
                def hlfunction(x): return self.Liquid_cp(x, pres)
            ...
            try:
                ...
                def eosphil(y): return math.log(self.Phil(y, pres, p0))
            ...
            try:
                ...
                def hlfunction(x): return self.Vl(temp, x)
    ...
    def Sub_dsv(self, temp, pres, t0=298.15, p0=100000, route="3", step=1000):  # !NOW EDITING!!!
        ...
        if route == "1":
            ...
            try:
                def gdiff(x): return self.Sub_dgv(x, pres, t0, p0, "2")
                # 数值微分方法,目前只设置了微分步长
    ...
    def Sub_dsl(self, temp, pres, t0=298.15, p0=100000, route="2"):
        ...
        if route == "1":
            ...
            def eosg(x): return self.Sub_dgl(x, pres, route="2")
    ...
    def Sub_dgv(self, temp, pres, t0=298.15, p0=100000, route="2"):
        ...
        if route == "1":
            ...
            # print eosphiv
            eosr = 8.3145 * temp * \
                math.log(eosphiv) + 8.3145 * temp * math.log(pres / p0)
            result = eosr * 1000
